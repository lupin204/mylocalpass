  ...
  <modal>
    ...
    <div slot="body">
      <form id="add-card-form" @submit.prevent="onSubmitNewCard" >
        <label>Key</label><input class="form-control" type="text" v-model="inputCardKey" ref="inputCardKey">
        <label>Value</label><input class="form-control" type="text"  v-model="inputCardValue" ref="inputCardValue">
      </form>
    </div>
    <div slot="footer">
...
export default {
  ...
  computed: {
    validateInput() {
      return !!this.inputCardKey.trim().length && !!this.inputCardValue.trim().length
    }
  },
  watch: {
    validateInput(check) {
      this.isValidInput = check
    }
  },
  mounted() {
    this.$refs.inputCardKey.focus()
  ...
  methods: {
    ...
    onSubmitNewCard() {
      ...
      if (!this.inputCardKey.trim()) return
      if (!this.inputCardValue.trim()) return

      //const pos = this.newCardPos()

      // const inputTitle = this.inputTitle;  const listId = this.listId;
      const { inputCardKey, inputCardValue, pos, listId } = this
      this.ADD_CARD({key: inputCardKey, value: inputCardValue, listId, pos})
