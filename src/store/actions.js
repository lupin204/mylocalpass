...
const actions = {
  ...
  ADD_CARD ({ state, dispatch }, { key, value, pos, listId }) {
    return card.create({ key, value, pos, listId })
      .then(_ => dispatch('FETCH_BOARD', state.board.id))
  },
  UPDATE_CARD ({ state, dispatch }, { id, key, value, pos, listId }) {
    return card.update(id, { pos, key, value, listId})
      .then(_ => dispatch('FETCH_BOARD', state.board.id))
  },
